...
(function bootstrapPickOptions(window, document) {
  ...
  function isTargetProduct() {
    return config.target.productNos.length === 0 || config.target.productNos.includes(currentProductNo());
  }
  ...
  function flavorImageUrl(image) {
    if (!image) return '';
    if (/^(?:https?:)?\/\//.test(image) || image.charAt(0) === '/') return image;
    const baseUrl = config.assets && config.assets.flavorImageBaseUrl || '';
    const version = config.assets && config.assets.version;
    return baseUrl + image + (version ? '?v=' + encodeURIComponent(version) : '');
  }
  ...
  function priceAdjustment(text) {
    const match = String(text || '').match(/\(([+-])?\s*([0-9][0-9,]*)원\)/);
    if (!match) return 0;
    const amount = Number(match[2].replace(/,/g, '')) || 0;
    return match[1] === '-' ? -amount : amount;
  }

  function resolveBundles(nativeOptions) {
    const grouped = new Map();
    nativeOptions.forEach(function groupOption(option) {
      if (!grouped.has(option.quantity)) grouped.set(option.quantity, []);
      grouped.get(option.quantity).push(option);
    });
    const resolved = Array.from(grouped.entries())
      .sort(function byQuantity(left, right) { return left[0] - right[0]; })
      .map(function createBundle(entry) {
        const meta = config.bundleMeta[String(entry[0])] || {};
        return {
          quantity: entry[0],
          label: entry[0] + '개입',
          options: entry[1].sort(function bySuffix(left, right) { return left.suffix - right.suffix; }),
          badge: meta.badge || '',
          badgeTone: meta.badgeTone || 'accent',
        };
      });
    const reference = resolved[0] && resolved[0].options[0];
    referenceUnitPrice = reference ? Math.round(reference.price / resolved[0].quantity) : 0;
    return resolved;
  }
  ...
  function createNativeAdapter(optionList, optionSelect, totalProducts) {
    ...
    function discoverOptions() {
      const priceElement = document.querySelector(config.selectors.productPrice);
      const basePrice = moneyValue(priceElement && priceElement.textContent);
      const seen = new Set();
      return Array.from(optionSelect.options).reduce(function collectOptions(result, selectOption) {
        if (config.option.emptyValues.includes(selectOption.value) || seen.has(selectOption.value)) return result;
        const match = selectOption.textContent.trim().match(config.option.labelPattern);
        if (!match) return result;
        const item = nativeItems().find(function matchingItem(candidate) {
          return candidate.getAttribute(config.option.valueAttribute) === selectOption.value;
        });
        if (!item) return result;
        seen.add(selectOption.value);
        result.push({
          quantity: Number(match[1]),
          suffix: Number(match[2]),
          value: selectOption.value,
          price: Math.max(0, basePrice + priceAdjustment(selectOption.textContent)),
        });
        return result;
      }, []);
    }
  ...
  function createController(adapter) {
    function bundleState(bundle) {
      ...
      const priceOption = next || availableOptions[availableOptions.length - 1] || null;
      const price = priceOption ? priceOption.price : 0;
      const unitPrice = bundle.quantity > 0 ? Math.round(price / bundle.quantity) : 0;
      const discountRate = referenceUnitPrice > 0
        ? Math.max(0, Math.round((1 - (unitPrice / referenceUnitPrice)) * 100))
        : 0;
      ...
      return {
        ...
        badge: bundle.badge,
        badgeTone: bundle.badgeTone,
      };
    }

    function getState() {
      return bundles.map(function stateForBundle(bundle) {
        return bundleState(bundle);
      });
    }

    function stateForOptionValue(value) {
      const bundle = bundles.find(function bundleForValue(candidate) {
    ...
    function add(quantity) {
      const bundle = bundles.find(function findBundle(candidate) {
  ...
  function createInterface(controller, optionTable, totalProducts, totalPrice) {
    ...
    function enforceQuantityLimits() {
      ...
      bundles.forEach(function enforceBundle(bundle) {
      ...
      if (corrected) {
        notice.textContent = '같은 개입수의 최대 선택 횟수를 초과해 자동 조정했습니다.';
    ...
    function cardMarkup(bundle) {
      return '<button type="button" class="pick-options__card" data-quantity="' + bundle.quantity + '">' +
        ...
        '<span class="pick-options__price-block">' +
          ...
          '<span class="pick-options__price-note"></span>' +
          '<span class="pick-options__badge pick-options__badge--danger" hidden></span></span>' +
    ...
    bundles.forEach(function renderBundle(bundle) {
    ...
    function renderFlavorPicker() {
      ...
      config.flavors.forEach(function renderFlavor(flavor) {
        ...
        if (flavor.image) {
          ...
          img.src = flavorImageUrl(flavor.image);
          img.alt = flavor.name + config.ui.imageAltSuffix;
          img.loading = 'lazy';
          img.decoding = 'async';
          img.width = 194;
          img.height = 232;
          img.addEventListener('error', function showImageFallback() {
            img.remove();
    ...
    function sync() {
      ...
      states.forEach(function syncCard(state) {
        ...
        const badge = card.querySelector('.pick-options__badge');
        ...
        card.querySelector('.pick-options__price').textContent =
          formatNumber.format(state.price) + config.ui.currency;
        card.querySelector('.pick-options__unit-price').textContent =
          '1개 : ' + formatNumber.format(state.unitPrice) + config.ui.currency;
        card.querySelector('.pick-options__price-note').textContent =
          state.discountRate > 0 ? '(' + state.discountRate + '% 할인)' : '';
        badge.hidden = !state.badge;
        badge.textContent = state.badge;
        badge.classList.toggle('pick-options__badge--accent', state.badgeTone === 'accent');
        badge.classList.toggle('pick-options__badge--danger', state.badgeTone === 'danger');
  ...
  function init() {
    ...
    bundles = resolveBundles(adapter.discoverOptions());
    if (bundles.length === 0) {
      console.error('[pick-options] Cafe24 옵션 라벨에서 개입수 구성을 찾지 못했습니다.');
      return;
    }
